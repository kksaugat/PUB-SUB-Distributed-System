...
var Subscriber = (function() {
  ...
  events.on("sub_unsubscribe", remove);
  ...
  function get(indx) {
    indx = indx || -1;
    if (indx != -1) return subscribers[indx];
    return subscribers;
  }
  ...
  function _notify(info) {
    subscribers.forEach(function(sub, indx) {
      ...
      if (sub.topics.indexOf(info.topic) != -1) sub.notifications.push(info);
    });
...
var Publisher = (function() {
  ...
  function get(indx) {
    indx = indx || -1;
    if (indx != -1) return publishers[indx];
    return publishers;
  }
